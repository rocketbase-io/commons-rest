...
| `required-annotations` | *(empty)* | annotation FQNs marking fields as required; replaces the default list |
| `zod-infer-type-export` | `false` | emit `export type X = z.infer<typeof XSchema>` next to each schema |
| `zod-infer-type-suffix` | *(empty)* | suffix for the inferred type name (e.g. `Zod` → `UserCreateCmdZod`) |
...
<Aside type="caution">
	`lombok.NonNull` and JetBrains `@NotNull` have CLASS retention — they are invisible at
	runtime and can never act as required-markers here.
</Aside>
